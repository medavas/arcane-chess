...
import { GameBoard, outputFenOfCurrentPosition } from 'src/features/game/engine/board.mjs';
...
export class SpellHandler {
    ...
    handleArcanaClick = (key: string): void => {
        ...
        if (key.startsWith('dyad')) {
            ...
            if (dyadName === key && dyadOwner === this.callbacks.getSelectedSide()) {
                ...
                this.callbacks.updateSpellState({ isDyadMove: false, normalMovesOnly: false });
                return;
            }
            ...
            if (state.isDyadMove && dyadClock === 0) {
                ...
                if (dests.size === 0) {
                    ...
                    this.callbacks.updateSpellState({ isDyadMove: true, normalMovesOnly: true });
                }
            }
            return;
        }
    ...
    isArcaneActive = (key: string, color?: string): boolean => {
        ...
        if (typeof color !== 'undefined' && color !== this.callbacks.getSelectedSide())
            return false;
